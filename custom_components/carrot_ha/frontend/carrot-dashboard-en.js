...
class CarrotDashboard extends HTMLElement {
  ...
  render(){
    ...
    const vehicleName=this.config?.vehicle_name||this.v?.vehicle_model||'Vehicle';
    const vehicleBrand=this.config?.vehicle_brand||(/ioniq/i.test(vehicleName)?'HYUNDAI':/volkswagen|meb|id\./i.test(vehicleName)?'VOLKSWAGEN':'');
    ...
    </style><ha-card><header class="top"><div><div class="brand">${vehicleBrand?esc(vehicleBrand)+' · ':''}CARROT HA</div><h1>${esc(vehicleName)}</h1></div><span class="badge ${state.key}"><i class="dot"></i>${badge}</span></header><nav class="nav">${[['overview','My car'],['trips','Trips'],['parking','Location'],['charge','Charging'],['vehicle','Status']].map(([key,label])=>`<button data-tab="${key}" class="${this.tab===key?'active':''}">${label}</button>`).join('')}</nav><main class="main">${this.error?`<div class="error">${esc(this.error)}</div>`:''}${this.body(v,trip,isTrip)}<footer class="foot"><div>Cloudflare · ${esc(v.cloud_status||'Checking connection')}<br>HA updated ${time(v.last_sync)}<br>Vehicle data received ${time(v.measured_at)}</div><button class="refresh">${this.busy?'Loading…':'↻ Refresh'}</button></footer></main></ha-card>`;
...
`;
  ...
  body(v,trip,isTrip){
    ...
    if(this.tab==='vehicle')return `<h2 class="section" style="margin-top:0">Vehicle information</h2><div class="allvalues">${[
      ['Distance this month',n(v.month_distance_km)+' km'],['Trips this month',n(v.month_trip_count,0)+'  trips'],['Battery level',n(v.soc_percent)+' %'],['SOC source',v.soc_source==='ovms_bms_pid_220101'?'OVMS BMS PID 22 0101':v.soc_source||'—'],['Battery energy',n(v.battery_kwh)+' kWh'],['Odometer',n(v.odometer_km,0)+' km'],['Range',n(v.range_km)+' km'],['HV battery',n(v.hv_voltage)+' V'],['12V battery',n(v.aux_voltage,2)+' V'],['Estimated BMS capacity',n(v.measured_capacity_kwh)+' kWh'],['SOC calculation capacity',n(v.soc_capacity_kwh)+' kWh'],['Outside temperature',n(v.outside_temp_c)+' °C'],['Air conditioning',v.ac_on==null?'Unavailable':v.ac_on?'On':'Off'],['Blower level',n(v.blower_level,0)],['Blower control voltage',n(v.blower_volt)+' V'],['Driver seat heating',n(v.seat_heat_left,0)],['Passenger seat heating',n(v.seat_heat_right,0)],['Recirculation signal',n(v.recirc,0)],['GPS accuracy',n(v.gps_accuracy_m)+' m'],['Current speed',n(v.speed_kph)+' km/h'],['Heading',n(v.bearing_deg)+' °'],['Recorded distance',n(v.recorded_distance_km)+' km'],['Driving assistance',v.enabled==null?'Unavailable':v.enabled?'Enabled':'Disabled']].map(([l,x])=>`<div class="table-row"><span>${l}</span><b>${esc(x)}</b></div>`).join('')}</div><p class="notice">— means unavailable. SOC calculation capacity applies only to energy-derived SOC; direct OVMS BMS SOC is not recalculated. Odometer and recorded distance are different values.</p><details><summary>Show all received data</summary><pre>${esc(JSON.stringify(v,null,2))}</pre></details>`;
    ...
    const tiles=isTrip?`${metric('Distance',n(trip.distance_m==null?null:trip.distance_m/1000,2),'km','map-marker-distance')}${metric('Duration',duration(trip.duration_s),'','timer-outline')}${metric('Average speed',n(trip.duration_s?trip.distance_m/trip.duration_s*3.6:null,0),'km/h','speedometer-medium')}${metric('Top speed',n(this.maxSpeed(trip.route),0),'km/h','speedometer')}`:
      `${metric('Battery level',n(v.soc_percent,0),'%','battery',v.soc_source==='ovms_bms_pid_220101'?'OVMS BMS SOC':`${n(v.battery_kwh)} / ${n(v.soc_capacity_kwh)} kWh`)}${metric('Odometer',n(v.odometer_km,0),'km','counter',v.stale?'Last measured':'Vehicle display')}${metric('Distance this month',n(v.month_distance_km),'km','routes',`${n(v.month_trip_count,0)} trips Trips`)}${metric('Estimated charging power',n(v.charge_power_w==null?null:v.charge_power_w/1000),'kW','ev-station',v.charging?'Charging increase detected':'Based on observations')}`;
