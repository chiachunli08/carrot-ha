...
`;
  ...
  batteryHistory(){
    ...
    const hours=d.hours.map((x,h)=>{const charging=!!d.charge_hours?.[h]||!!x?.charging;return `<div title="${h}시 ${x?(x.last_known?'마지막 확인값 ':'실측 ')+Math.round(x.soc)+'%':'SOC 기록 없음'}${charging?' · 해당 시간 충전 기록 있음':''}" class="hour ${charging?'charging':x?.last_known?'last-known':x?.driving?'driving':'parked'} ${!charging&&x?(x.soc<15?'soc-critical':x.soc<30?'soc-low':''):''}">${charging?'<em aria-hidden="true"><svg viewBox="0 0 32 40"><path d="M19 5 Q21 3 20 7 L17 17 H25 Q27 17 25 20 L13 35 Q11 37 12 33 L15 23 H7 Q5 23 7 20 Z" fill="#5ad46d" stroke="var(--bolt-outline)" stroke-width="2.2" stroke-linejoin="round" stroke-linecap="round"/></svg></em>':''}${x?`<i style="height:${x.soc}%"></i>`:''}</div>`;}).join('');
...
if(!customElements.get('carrot-history-card'))customElements.define('carrot-history-card',CarrotDashboard);
if(!customElements.get('carrot-dashboard-card'))customElements.define('carrot-dashboard-card',class extends CarrotDashboard{});
window.customCards=window.customCards||[];window.customCards.push({type:'carrot-dashboard-card',name:'MEB 차량 대시보드',description:'지도·주행·주차·충전·차량 정보'});
