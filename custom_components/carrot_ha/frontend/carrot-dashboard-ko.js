...
class CarrotDashboard extends HTMLElement {
  ...
  render(){
    ...
    const vehicleName=this.config?.vehicle_name||this.v?.vehicle_model||'차량';
    ...
    </style><ha-card><header class="top"><div><div class="brand">${vehicleBrand?esc(vehicleBrand)+' · ':''}CARROT HA</div><h1>${esc(vehicleName)}</h1></div><span class="badge ${state.key}"><i class="dot"></i>${badge}</span></header><nav class="nav">${[['overview','내 차'],['trips','주행'],['parking','위치'],['charge','충전'],['vehicle','상태']].map(([key,label])=>`<button data-tab="${key}" class="${this.tab===key?'active':''}">${label}</button>`).join('')}</nav><main class="main">${this.error?`<div class="error">${esc(this.error)}</div>`:''}${this.body(v,trip,isTrip)}<footer class="foot"><div>Cloudflare · ${esc(v.cloud_status||'연결 확인 중')}<br>HA 업데이트 ${time(v.last_sync)}<br>차량 정보 수신 ${time(v.measured_at)}</div><button class="refresh">${this.busy?'조회 중…':'↻ 새로고침'}</button></footer></main></ha-card>`;
...
`;
  ...
  body(v,trip,isTrip){
    ...
    if(this.tab==='vehicle')return `<h2 class="section" style="margin-top:0">차량 정보</h2><div class="allvalues">${[
      ['이번 달 주행거리',n(v.month_distance_km)+' km'],['이번 달 주행 횟수',n(v.month_trip_count,0)+' 회'],['배터리 잔량',n(v.soc_percent)+' %'],['SOC 출처',v.soc_source==='ovms_bms_pid_220101'?'OVMS BMS PID 22 0101':v.soc_source||'—'],['배터리 에너지',n(v.battery_kwh)+' kWh'],['총 주행거리',n(v.odometer_km,0)+' km'],['주행가능거리',n(v.range_km)+' km'],['고전압 배터리',n(v.hv_voltage)+' V'],['12V 배터리',n(v.aux_voltage,2)+' V'],['BMS 용량 추정',n(v.measured_capacity_kwh)+' kWh'],['SOC 계산 용량',n(v.soc_capacity_kwh)+' kWh'],['외부 온도',n(v.outside_temp_c)+' °C'],['에어컨',v.ac_on==null?'정보 없음':v.ac_on?'작동':'꺼짐'],['송풍 단계',n(v.blower_level,0)],['송풍 제어 전압',n(v.blower_volt)+' V'],['운전석 열선',n(v.seat_heat_left,0)],['조수석 열선',n(v.seat_heat_right,0)],['내기순환 신호',n(v.recirc,0)],['GPS 정확도',n(v.gps_accuracy_m)+' m'],['현재 속도',n(v.speed_kph)+' km/h'],['진행 방향',n(v.bearing_deg)+' °'],['기록된 누적 거리',n(v.recorded_distance_km)+' km'],['주행 보조',v.enabled==null?'정보 없음':v.enabled?'활성':'비활성']].map(([l,x])=>`<div class="table-row"><span>${l}</span><b>${esc(x)}</b></div>`).join('')}</div><p class="notice">— 는 정보 없음입니다. SOC 계산 용량은 에너지 기반 SOC에만 적용되며 OVMS 직접 BMS SOC는 다시 계산하지 않습니다. 총 주행거리와 기록된 누적 거리는 서로 다른 값입니다.</p><details><summary>수신 데이터 전체 보기</summary><pre>${esc(JSON.stringify(v,null,2))}</pre></details>`;
    ...
    const tiles=isTrip?`${metric('주행거리',n(trip.distance_m==null?null:trip.distance_m/1000,2),'km','map-marker-distance')}${metric('주행시간',duration(trip.duration_s),'','timer-outline')}${metric('평균속도',n(trip.duration_s?trip.distance_m/trip.duration_s*3.6:null,0),'km/h','speedometer-medium')}${metric('최고속도',n(this.maxSpeed(trip.route),0),'km/h','speedometer')}`:
      `${metric('배터리 잔량',n(v.soc_percent,0),'%','battery',v.soc_source==='ovms_bms_pid_220101'?'OVMS BMS SOC':`${n(v.battery_kwh)} / ${n(v.soc_capacity_kwh)} kWh`)}${metric('총 주행거리',n(v.odometer_km,0),'km','counter',v.stale?'마지막 측정값':'차량 계기판')}${metric('이번 달 주행거리',n(v.month_distance_km),'km','routes',`${n(v.month_trip_count,0)}회 주행`)}${metric('충전 전력 · 추정',n(v.charge_power_w==null?null:v.charge_power_w/1000),'kW','ev-station',v.charging?'충전 증가 감지':'관측값 기준')}`;
