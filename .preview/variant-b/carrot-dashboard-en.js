...
/* Variant B: merge adjacent charging backgrounds into one green band; bars, gaps and original bolt icon stay unchanged */
.hours .charge-run-wrap{position:relative;display:flex;flex:1;gap:4px;min-width:0;background:rgba(90,212,109,.18);border-radius:7px 7px 0 0}
...
@container(max-width:700px){.hours .charge-run-wrap{gap:2px}}
...
`;
  ...
  batteryHistory(){
    ...
    const hours=(()=>{const runs=[];let start=null;for(let h=0;h<d.hours.length;h++){const ch=!!d.charge_hours?.[h]||!!d.hours[h]?.charging;if(ch&&start===null)start=h;if(!ch&&start!==null){runs.push([start,h-1]);start=null;}}if(start!==null)runs.push([start,d.hours.length-1]);let out='',ridx=0;for(let h=0;h<d.hours.length;h++){const x=d.hours[h],run=runs[ridx],charging=!!d.charge_hours?.[h]||!!x?.charging;const boltHour=run?run[0]+Math.floor((run[1]-run[0])/2):-1;if(run&&h===run[0])out+=`<div class="charge-run-wrap" style="flex:${run[1]-run[0]+1}">`;out+=`<div title="${h}:00 ${x?(x.last_known?'Last known ':'Measured ')+Math.round(x.soc)+'%':'No SOC record'}${charging?' · Charging recorded during this hour':''}" class="hour ${charging?'charging':x?.last_known?'last-known':x?.driving?'driving':'parked'} ${!charging&&x?(x.soc<15?'soc-critical':x.soc<30?'soc-low':''):''}">${charging&&h===boltHour?'<em aria-hidden="true"><svg viewBox="0 0 32 40"><path d="M19 5 Q21 3 20 7 L17 17 H25 Q27 17 25 20 L13 35 Q11 37 12 33 L15 23 H7 Q5 23 7 20 Z" fill="#5ad46d" stroke="var(--bolt-outline)" stroke-width="2.2" stroke-linejoin="round" stroke-linecap="round"/></svg></em>':''}${x?`<i style="height:${x.soc}%"></i>`:''}</div>`;if(run&&h===run[1]){out+='</div>';ridx++;}}return out;})();
